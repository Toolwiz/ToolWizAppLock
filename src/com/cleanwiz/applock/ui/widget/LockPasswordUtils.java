...
public class LockPasswordUtils {
	...
	private static class PasswordFileObserver extends FileObserver {
		...
		public void onEvent(int event, String path) {
			if (LOCK_PASSWORD_FILE.equals(path)) {
				LogUtil.d(TAG, "lock password file changed");
	...
	public void saveLockPassword(String password, int quality,
		...
		try {
			...
			// provider
			LogUtil.e(TAG, "Unable to save lock pattern to "
	...
	public byte[] passwordToHash(String password) {
		...
		try {
			...
			LogUtil.i(TAG, "Failed to encode string because of missing algorithm: "
					+ algo);
		}
		return hashed;
	}
	...
	private String getSalt() {
		...
		if (salt == 0) {
			try {
				...
				LogUtil.i(TAG, "Initialized lock password salt");
			} catch (NoSuchAlgorithmException e) {
